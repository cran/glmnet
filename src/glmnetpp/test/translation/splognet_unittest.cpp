...
protected:
    ...
    void check_pack(const SpLognetPack& actual,
                    const SpLognetPack& expected)
    {
...
TEST_P(sp_lognet_fixture, sp_lognet_test)
{
    SpLognetPack actual(
            maxit, nx, ne, nlam, alpha, flmin, isd, intr, kopt,
            X, y, g, ulam, vp, cl, jd);
    SpLognetPack expected(actual);
    run(actual, expected, 0, 1); 
    check_pack(actual, expected);
}

INSTANTIATE_TEST_SUITE_P(
    SpLognetSuite, sp_lognet_fixture,
    testing::Combine(
