...
cox.path <- function(x, y, weights=NULL, offset=NULL, 
  ...
  exclude <- c(exclude, which(const_vars == 0))
  exclude <- sort(unique(exclude))
  if (length(exclude) == nvars) stop("All used predictors have zero variance")
...
get_cox_lambda_max <- function(x, y, alpha, weights = rep(1, nrow(x)),
  ...
  if (length(vp_zero) > 0) {
    ...
    if (length(unique(strata)) == 1) {
      fit <- survival::coxph(y ~ offset(offset) + tempx, weights = weights)
    } else {
  ...
  if (inherits(x, "sparseMatrix")) {
    ...
    g <- unlist(lapply(1:nvars, function(j)
      abs(sum(null_grad * (x[, j] - xm[j]) / xs[j]))))
  } else {
    g <- unlist(lapply(1:nvars, function(j)
      abs(sum(null_grad * x[, j]))))
  }
